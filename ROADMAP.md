...
| ID | Status | Dependencies | Deliverable and acceptance criteria |
|---|---|---|---|
| `IMG-DEC-001` | Planned | Gate P2, `MOD-001` | Resolve the image-decoder and image-limit portion of `D-008`, including formats, metadata/orientation, color/alpha, native dependencies, licenses, and hard limits. `docs/IMAGE_DECODER_EVIDENCE.md` contains pre-gate isolated `image` 0.25.10/OpenCV color-orientation observations plus bounded no-AVX/no-FMA QEMU execution for both the minimal `image` route and the direct `jpeg-decoder` 0.3.2 + `png` 0.18.1 route; no decoder or input policy is selected. |
