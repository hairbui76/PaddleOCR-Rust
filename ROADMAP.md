...
| `TEST-002` | In progress | `FND-002`, `FND-003` | Foundation regressions cover empty/oversized encoded input, invalid dimensions, NaN/infinity, degenerate quadrilaterals, Unicode, resource bounds, reviewed self-authored DB strict-threshold, CTC greedy-path, and minimum-area-candidate fixture cases plus malformed-value rejection, affine/projective transform round trips across diverse convex grids plus a thin skewed crop and non-finite sampler errors, five selected source-to-warp checks and sixty-five captured OpenCV pre-rotation warp-to-source boundary/interior checks across all thirteen crop cases, binary half-tie detector scaling, bounded minimum-area candidate cases for concave/collinear/rotated/triangular polygons, private interleaved crop bytes/borders/checked `f32` cubic sampling/rotation/allocation limits, thirteen reviewed BGR OpenCV crop-oracle cases including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, tall-thin, high-variation cubic-rounding, cubic-weight-construction, and sampling-matrix paths, classic stable-order edge cases, and M2 Must contract-coverage drift. Tensor properties and broader OpenCV pixel-oracle coverage await their owning implementations. |
