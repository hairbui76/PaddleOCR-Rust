...
| `DB-002` | Done | `DB-001`, `FND-002`, `FND-003` | `src/db.rs` now scans a copied bounded `BinaryBitmap` with a deterministic row-major-seed 8-connected scanline frontier and returns only inclusive component bounds plus pixel counts. It permits at most `1,000` components and `1,000,000` pending seeds per component, returning typed resource errors instead of silently dropping regions or allocating without a limit. The reviewed self-authored `classic-v1-db-components` fixture covers diagonal connectivity and order; source tests cover empty masks, an independent exhaustive 3×3 reference, and excess isolated components. This precursor does not claim OpenCV `findContours`/`RETR_LIST`/`CHAIN_APPROX_SIMPLE`, hierarchy or holes, contour ordering, geometry, scores, offsets, detector output, model ABI, or detector support; those remain `DET-003` after its gates. |
