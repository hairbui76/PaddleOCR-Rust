...
| `FUZZ-001` | In progress | Pure processing surfaces | `src/fuzz.rs` and the feature-gated `fuzz-primitives` stdin target now drive checked public types plus bounded DB, CTC, geometry, and crop kernels from at most `16 KiB` of bytes. Deterministic seeds cover the initial driver; manifests/configs/schema/document parsers, a mutator campaign, and native decoder corpora remain outstanding. |
