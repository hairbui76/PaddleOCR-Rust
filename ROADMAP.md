...
| `TEST-001` | Done | `FND-001`, `COMP-001` | Added fixture provenance/golden policy, offline foundation tests, and `tests/fixture_integrity.rs`. The latter parses all four committed metadata records, requires the pinned baseline/provenance/profile/test/limitation fields and null artifacts, rejects symlinked fixture paths, verifies direct file SHA-256 values, and verifies the crop capture, inverse-mapping CSV, all fourteen base64 BGR payloads/shapes, and aggregate digests. Its `base64`, `serde_json`, and scalar `sha2` `force-soft` dependencies are dev-only. No upstream asset was copied. |
| `TEST-002` | In progress | `FND-002`, `FND-003` | Foundation regressions cover empty/oversized encoded input, invalid dimensions, NaN/infinity, degenerate quadrilaterals, Unicode, resource bounds, reviewed self-authored DB strict-threshold, CTC greedy-path including positive/negative finite-extreme aggregate-overflow means, and minimum-area-candidate fixture cases plus malformed-value rejection, affine/projective transform round trips across diverse convex grids plus a thin skewed crop and non-finite sampler errors, five selected source-to-warp checks and seventy captured OpenCV pre-rotation warp-to-source boundary/interior checks across all fourteen crop cases, binary half-tie detector scaling, bounded minimum-area candidate cases for concave/collinear/rotated/triangular polygons, private interleaved crop bytes/borders/checked `f32` cubic sampling/rotation/allocation limits, a deterministic source-level projective grid for all one-through-four interleaved channels and the negative-subnormal `f32` upper-phase endpoint, separate 2,048-case bounded affine and true-projective trapezoid crop grids that check plan dimensions and interleaved byte counts without panic, fourteen reviewed BGR OpenCV crop-oracle cases including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, tall-thin, high-variation cubic-rounding, cubic-weight-construction, sampling-matrix, and perspective-LU paths, classic stable-order edge cases, M2 Must contract-coverage drift, and a static no-AVX/no-FMA QEMU run of the current 65-library, 3-contract, and 3-foundation-test suite. Tensor properties and broader OpenCV pixel-oracle coverage await their owning implementations. |
