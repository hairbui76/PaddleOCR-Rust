...
| `FIX-001` | In progress | `ORACLE-001` | `docs/FIXTURE_AND_TOLERANCE_PLAN.md` defines the legal M2 corpus, exact required coverage, metadata, and blocked model-backed classes. The reviewed self-authored crop oracle, minimum-area candidate, DB-boundary bitmap, DB-component, CTC greedy-path, and `classic-v1-image-inputs` corpus provide offline representatives. The last records fifteen valid PNG/JPEG inputs, five negatives, and version-recorded OpenCV BGR evidence without selecting decoder behavior; remaining decoder and model-backed representatives are still gated. Do not copy unclear upstream fonts/assets. |
| `TOL-001` | In progress | `CTR-001`, `FIX-001`, `QUAL-001` | `docs/FIXTURE_AND_TOLERANCE_PLAN.md` binds unit, image-input-oracle, end-to-end, tensor, and determinism profiles to the predeclared budgets. `m2-image-input-oracle-v1` preserves fixed capture evidence but deliberately defers Rust exact/tolerance/unsupported classification to `D-008`; bind each committed fixture to its actual profile and artifact evidence before completion. |
