...
| `TEST-001` | Done | `FND-001`, `COMP-001` | Added fixture provenance/golden policy, offline foundation tests, and `tests/fixture_integrity.rs`. The latter parses all five committed metadata records, requires the pinned baseline/provenance/profile/test/limitation fields and null artifacts, rejects symlinked fixture paths, verifies direct file SHA-256 values, and verifies the crop capture, inverse-mapping CSV, the exact fifteen-case base64 BGR payload/shape corpus, and aggregate digests. Its `base64`, `serde_json`, and scalar `sha2` `force-soft` dependencies are dev-only. No upstream asset was copied. |
...
| ID | Status | Dependencies | Deliverable and acceptance criteria |
|---|---|---|---|
| `IMG-DEC-001` | Planned | Gate P2, `MOD-001` | Resolve the image-decoder and image-limit portion of `D-008`, including formats, metadata/orientation, color/alpha, native dependencies, licenses, and hard limits. `docs/IMAGE_DECODER_EVIDENCE.md` contains pre-gate isolated `image` 0.25.10/OpenCV color-orientation observations, bounded no-AVX/no-FMA QEMU execution, and a self-authored 1,438-attempt mutation corpus, plus direct `jpeg-decoder` 0.3.2 + `png` 0.18.1 no-AVX evidence; no decoder or input policy is selected. |
