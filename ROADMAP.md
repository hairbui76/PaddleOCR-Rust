...
| ID | Status | Dependencies | Deliverable and acceptance criteria |
|---|---|---|---|
| `IMG-DEC-001` | In progress | Gate P2 and `MOD-001` to close; pre-gate evidence only is permitted | Resolve the image-decoder and image-limit portion of `D-008`, including formats, metadata/orientation, color/alpha, native dependencies, licenses, and hard limits. `docs/IMAGE_DECODER_EVIDENCE.md` contains isolated `image` 0.25.10/OpenCV color-orientation observations, bounded no-AVX/no-FMA QEMU execution, a self-authored 1,438-attempt mutation corpus, direct `jpeg-decoder` 0.3.2 + `png` 0.18.1 no-AVX evidence and full self-authored input-oracle replay/supply-chain audit, and the reviewed `classic-v1-image-inputs` OpenCV capture fixture. The direct pair is a non-leading comparison route because its JPEG result has a maximum component delta of 36 in that finite corpus. No decoder or input policy is selected. |
