...
| `LIC-001` | In progress | `MOD-001`, `DEC-003` | `docs/LICENSE_REVIEW.md` records preliminary revision-specific model-card/listing evidence, local package observations, source-level dictionary-index evidence, and explicit gaps for the M2 candidates. Verify candidate model, dictionary, tokenizer, fixture, conversion-tool, and retained-notice terms before the final distribution decision; source-level ABI evidence does not approve an asset, and unclear assets are never distributed. |
