...
Status: In progress; one component crop oracle fixture is committed, but no end-to-end model fixture or capture has been added
...
| `classic-v1-db-segmentation` | DB map | Strict `map_value > 0.3` binary output, exact equality exclusion, row-major order, and malformed/non-finite map rejection. | Exact zero/one bytes or typed model tensor-contract error. | `DB-001`. |
| `classic-v1-crop-pixels` | Crop | Perspective-map sampling, replicated source borders, channel preservation, cubic subpixel behavior, output allocation limits, and `np.rot90` byte order. | Exact for self-authored fixed-kernel vectors and ten reviewed OpenCV 5.0.0 BGR cases, including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, and tall-thin cases; further approved captures are required before claiming upstream pixel equivalence. | `CROP-001`, `ORACLE-001`. |
| `classic-v1-ctc-basic` | CTC | Blank index 0, immediate duplicate removal before blank removal, selected-max score mean, and empty selection score. | Exact retained class indexes and score in numeric unit arithmetic; text follows after dictionary validation. | `CTC-001`, `REC-003`. |
| `classic-v1-ctc-tie-and-dictionary` | CTC | Lowest-index argmax tie, out-of-range dictionary index, and malformed/non-finite tensor behavior. | Exact retained indexes or typed model error; exact decoded text follows dictionary validation. | `CTC-001`, `REC-003`. |
...
|---|---|---|---|---|---|---|
| `m2-unit-v1` | Self-authored geometry and CTC unit fixtures | Exact | Exact where the operation is integer-defined | Exact deterministic arithmetic result | Not applicable | Exact error variant/category and boundary condition. |
