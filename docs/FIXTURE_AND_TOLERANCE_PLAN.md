...
| `classic-v1-crop-pixels` | Crop | Perspective-map sampling, replicated source borders, channel preservation, cubic subpixel behavior, output allocation limits, and `np.rot90` byte order. | Exact for self-authored fixed-kernel vectors and thirteen reviewed OpenCV 5.0.0 BGR cases, including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, tall-thin, high-variation cubic-rounding, cubic-weight-construction, and sampling-matrix cases; further approved captures are required before claiming upstream pixel equivalence. | `CROP-001`, `ORACLE-001`. |
