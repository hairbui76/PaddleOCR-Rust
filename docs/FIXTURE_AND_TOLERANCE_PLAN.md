...
Status: In progress; seven offline fixture sets are present, including self-authored image-input and scalar crop oracles, but no end-to-end model fixture or capture has been added
...
does the same for the separately recorded scalar OpenCV grid. Focused source
...
The current seven committed offline fixture directories are also covered by
...
valid/negative encoded bytes and BGR output aggregates. This protects
fixture-record consistency only; it neither approves a model artifact nor
upgrades the component fixtures to end-to-end evidence.
...
| `classic-v1-crop-pixels` | Crop | Perspective-map sampling, replicated source borders, channel preservation, cubic subpixel behavior, output allocation limits, and `np.rot90` byte order. | Exact for self-authored fixed-kernel vectors, fifteen reviewed default-OpenCV 5.0.0 BGR cases, and 24 separately reviewed `cv2.setUseOptimized(False)` BGR grid cases with 3–16-pixel source sides, border crossings, wide/tall/balanced extents, and perspective perturbations; further approved captures are required before claiming upstream pixel equivalence. | `CROP-001`, `ORACLE-001`. |
