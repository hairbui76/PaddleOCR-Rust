...
Status: In progress; six offline fixture sets are present, including a self-authored image-input oracle, but no end-to-end model fixture or capture has been added
...
The current six committed offline fixture directories are also covered by
...
digest plus the image-input capture's valid/negative encoded bytes and BGR
output aggregates. This protects fixture-record consistency only; it neither
approves a model artifact nor upgrades the component fixtures to end-to-end
evidence.
...
| `classic-v1-crop-pixels` | Crop | Perspective-map sampling, replicated source borders, channel preservation, cubic subpixel behavior, output allocation limits, and `np.rot90` byte order. | Exact for self-authored fixed-kernel vectors and fifteen reviewed OpenCV 5.0.0 BGR cases, including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, tall-thin, high-variation cubic-rounding, cubic-weight-construction, sampling-matrix, perspective-LU, and scalar nearest-even rounding cases; further approved captures are required before claiming upstream pixel equivalence. | `CROP-001`, `ORACLE-001`. |
