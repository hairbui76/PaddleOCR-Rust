...
Status: In progress; eight offline fixture sets are present, including a narrow
reviewed classic-ONNX no-text oracle; other end-to-end, decoder, and tensor
...
| End-to-end image goldens | `classic-v1-e2e-*` | Original contributor material or an explicitly redistributable source only. | Fixture hash/provenance review, exact model/dictionary hashes, isolated oracle capture, and output review. | In progress; `classic-v1-e2e-no-text/` records one 3-by-2 self-authored PNG and its previously captured BGR bytes, with a two-fresh-process CPU classic `TextSystem` ONNX oracle result of `lines: []`. It retains no weights, dictionary entries, tensors, or source checkout. It does not choose a Rust decoder/runtime or prove text, geometry, score, ordering, or error behavior. |
...
The current eight committed offline fixture directories are also covered by
...
valid/negative encoded bytes and BGR output aggregates. The no-text gate also
pins the exact candidate revisions/hashes and terms-review reference, source
record digest, two matching fresh-process output digests, and fixture-only
native-result projection. This protects fixture-record consistency only; it
does not select a Rust backend/decoder or establish Rust model support.
...
| `m2-e2e-v1` | Captured classic end-to-end fixture with the same verified artifact | Exact UTF-8 and final order | Each corresponding quad coordinate absolute error `<= 1.0` px | Absolute error `<= 0.001` | Not applicable | Exact success/error classification; no partial result. The initial no-text fixture exercises only the exact empty `lines` case, so its geometry and score budgets are intentionally unused. |
