...
|---|---|---|---|---|
| `M2-GEO-001` | [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md) defines resize, DB-coordinate, reading-order, crop, border, rotation, and source-coordinate rules; [FIXTURE_AND_TOLERANCE_PLAN.md](FIXTURE_AND_TOLERANCE_PLAN.md) defines unit/oracle evidence. | Private `src/geometry.rs` and `src/crop.rs` implement checked geometry, a bounded convex-hull minimum-area candidate over checked polygon vertices, and a bounded crop candidate with self-authored unit vectors. `tests/fixtures/classic-v1-geometry-min-area-candidate/` records its reviewed concave/collinear candidate representative. Crop geometry includes a skewed 16,000-by-1-pixel homography round trip, non-finite sampler-coordinate rejection, two deterministic 2,048-case bounded affine/projective-trapezoid source-level grids, five selected source-to-warp checks, and seventy-five OpenCV-captured pre-rotation warp-to-source boundary/interior checks across the fifteen reviewed crop cases. `tests/fixtures/classic-v1-crop-oracle/` supplies fifteen exact BGR OpenCV 5.0.0 crop regressions, including non-linear fractional interior, all-side-border, tall-projective, eighth-pixel phase, one-pixel, tall-thin, high-variation cubic-rounding, cubic-weight-construction, sampling-matrix, perspective-LU, and scalar nearest-even rounding cases. Neither candidate is a general OpenCV-equivalence claim. | Broader approved OpenCV pixel coverage; decoded-image/color semantics; verified OpenCV minimum-area semantics; and `DET-003` contour/offset postprocessing after its detector/runtime gates. | Private only. No decoded-image or upstream pixel/rectangle-equivalence claim. |
| `M2-DET-001` | The detector preprocessing and DB map/postprocessing sections of [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md); candidate tensor observations in [MODEL_CANDIDATES.md](MODEL_CANDIDATES.md). | `src/db.rs` implements `DB-001`: private checked strict segmentation over one borrowed finite row-major map, and `DB-002`: a bounded 8-connected component scan with row-major seeds, inclusive bounds, and pixel counts. The self-authored `classic-v1-db-map-boundaries` and `classic-v1-db-components` fixtures record those exact private outputs. No detector tensor ABI, inference adapter, OpenCV contour/hierarchy/simplification behavior, scoring, offset, or DB box postprocessor exists. | `IMG-DEC-001`, `IMG-002`, `TEN-001`, accepted local artifact/ABI, runtime qualification, and the `DET-001`/`DET-002` gates. Neither the candidate's DB thresholds nor its serialized `DetResizeForTest: null` path (pinned source default `736/min`) may replace the frozen M2 `0.3`/`0.6`/`1.5`/`1000`, `960/max` profile. | Absent; no detector support claim. |
| `M2-REC-001` | The recognition/CTC section of [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md); candidate recognizer/dictionary evidence in [MODEL_CANDIDATES.md](MODEL_CANDIDATES.md). | `src/ctc.rs` implements `CTC-001`: private checked numeric greedy indexes with classic tie/repeat/blank/mean behavior. The self-authored `classic-v1-ctc-greedy-path` fixture records its numeric path/mean behavior. The exact local candidate now has source-level blank/entry/space index-construction evidence in [LOCAL_ONNX_CANDIDATE_INSPECTION.md](LOCAL_ONNX_CANDIDATE_INSPECTION.md), but no recognizer tensor batching/inference, Rust dictionary, text decoder, or language behavior exists. | `IMG-002`, `TEN-001`, accepted terms, runtime-output/dictionary validation, runtime qualification, and `REC-001`/`REC-002`. | Absent; no language or CTC-output compatibility claim. |
| `M2-OCR-001` | The classic system sequence in [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md) and result invariants in [API_CONTRACT.md](API_CONTRACT.md). | No public `Ocr` engine or pipeline orchestration exists. `tests/fixtures/classic-v1-e2e-no-text/` preserves one classic CPU/ONNX `lines: []` result; `tests/fixtures/classic-v1-e2e-reading-order/` preserves one self-authored four-line text/order/quad/confidence result; `tests/fixtures/classic-v1-e2e-tall-crop/` preserves one self-authored rotated-text result plus the source `get_rotate_crop_image` branch dimensions; and `tests/fixtures/classic-v1-e2e-unicode/` preserves one self-authored CJK phrase with an exact UTF-8 result. All pin candidate hashes and use a BGR-input boundary. | Gate P5 components, `CROP-001` oracle/decoder evidence, `OCR-001` through `OCR-003`, and offline end-to-end fixtures. | Absent; the four fixtures are evidence data only and the binary intentionally rejects OCR use. |
...
4. The exact candidate now has source-level CTC index-construction evidence for
   its 18,710-class count, blank, appended space, and duplicate-free entry
   stream. It does not prove runtime-output, malformed-index, text-normalization,
   or language semantics.
5. Four legally reviewed model-backed BGR-boundary oracles exist, but no Rust
   OCR path consumes them. `classic-v1-e2e-no-text` covers only `lines: []`;
   `classic-v1-e2e-reading-order` covers one synthetic four-line text, order,
   quadrilateral, and confidence result; and `classic-v1-e2e-tall-crop` covers
   one synthetic `get_rotate_crop_image` rotation branch. The Unicode fixture
   covers only one CJK phrase rendered with one reviewed external font; it does
   not establish font fallback, shaping, bidi, or normalization behavior.
   Threshold, malformed-input, resource-limit, decoder, raw-tensor, broader
   tall-crop, and actual Rust differential qualifications remain absent; these
   fixtures do not close the gap.
