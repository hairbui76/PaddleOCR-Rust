...
| `M2-OCR-001` | The classic system sequence in [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md) and result invariants in [API_CONTRACT.md](API_CONTRACT.md). | No public `Ocr` engine or pipeline orchestration exists. `tests/fixtures/classic-v1-e2e-no-text/` preserves one classic CPU/ONNX `lines: []` result, while `tests/fixtures/classic-v1-e2e-reading-order/` preserves one self-authored four-line text/order/quad/confidence result; both pin candidate hashes and use a BGR-input boundary. | Gate P5 components, `CROP-001` oracle/decoder evidence, `OCR-001` through `OCR-003`, and offline end-to-end fixtures. | Absent; the two fixtures are evidence data only and the binary intentionally rejects OCR use. |
...
5. Two legally reviewed model-backed BGR-boundary oracles exist, but no Rust
   ...
   quadrilateral, and confidence result. Threshold, Unicode, malformed-input,
   resource-limit, decoder, raw-tensor, tall-crop, and actual Rust differential
   qualifications remain absent; these fixtures do not close the gap.
