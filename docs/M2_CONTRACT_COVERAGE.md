...
| `M2-OCR-001` | The classic system sequence in [CLASSIC_OCR_CONTRACT.md](CLASSIC_OCR_CONTRACT.md) and result invariants in [API_CONTRACT.md](API_CONTRACT.md). | No public `Ocr` engine or pipeline orchestration exists. `tests/fixtures/classic-v1-e2e-no-text/` now preserves one classic CPU/ONNX oracle result with `lines: []`, exact candidate hashes, and a BGR-input boundary. | Gate P5 components, `CROP-001` oracle/decoder evidence, `OCR-001` through `OCR-003`, and offline end-to-end fixtures. | Absent; the no-text fixture is evidence data only and the binary intentionally rejects OCR use. |
...
5. One legally reviewed model-backed `classic-v1-e2e-no-text` BGR-boundary
   oracle exists, but no Rust OCR path consumes it. Text, geometry, confidence,
   reading order, threshold, Unicode, malformed-input, resource-limit, decoder,
   and raw-tensor qualifications remain absent; the narrow no-text result does
   not close this gap.
