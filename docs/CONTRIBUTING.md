...
Set every one of these first. A gate whose variable is missing **fails** rather
than skipping, so a partial environment looks like a broken project:

| Variable | Needed by |
|---|---|
| `PADDLEOCR_RUST_ORT_DYLIB` | every provisioned gate |
| `PADDLEOCR_RUST_DETECTOR_ONNX` | `G1`, `G3`, engine reuse, adapter fingerprint, `E2E-001` |
| `PADDLEOCR_RUST_RECOGNIZER_ONNX` | the same |
| `PADDLEOCR_RUST_DICTIONARY` | the same |
| `PADDLEOCR_RUST_PREPROCESS_CAPTURE` | `PRE-001`, regenerated by `tools/capture_preprocess_oracle.py` |
| `PADDLEOCR_RUST_ORIENTATION_ONNX` | `DOCORI-001`, the text-line orientation gate |
| `PADDLEOCR_RUST_DOC_ORIENTATION_ONNX` | `DOCPIPE-001`, the rotated-page gate |
| `PADDLEOCR_RUST_ROTATED_PAGE` | `DOCPIPE-001`, a 90-degree rotation of the reading-order fixture |
