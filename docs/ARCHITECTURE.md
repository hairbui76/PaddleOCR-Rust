...
| `db` | Private checked borrowed one-map DB segmentation with fixed strict M2 threshold, bounded byte-mask output, and a bounded row-major-seed 8-connected component scan. | `DB-001`/`DB-002` only; no runtime tensor ABI, OpenCV contour semantics, scoring, polygon expansion, boxes, or detector support. |
