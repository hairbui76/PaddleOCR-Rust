...
The deterministic tests are smoke/regression checks, not a fuzzing campaign.
They exercise empty input, 128 fixed byte streams, an input larger than the
`16 KiB` consumption limit, and 4,096 generated byte streams whose lengths
vary from zero through the driver limit.
