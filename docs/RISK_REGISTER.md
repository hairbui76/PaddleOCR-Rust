...
|---|---|---|---|---|
| `RISK-001` | Model weights, dictionaries, fonts, fixtures, or conversion output may lack redistributable terms. | High | No assets are bundled or downloaded automatically; retain provenance per artifact. | `LICENSE_REVIEW.md` is the in-progress `LIC-001` evidence record; `LIC-002` and manifest review must still establish final terms and notices. |
