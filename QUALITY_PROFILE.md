...
| Raw tensor qualification | Maximum absolute and relative element error at most `1e-4` for components that are expected to be numerically equivalent; an approved exception must identify the operator and tolerance. | P3 backend/raw-output comparison. |
